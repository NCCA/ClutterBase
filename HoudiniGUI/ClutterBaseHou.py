...
class ClutterBaseDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.mesh_location.setItemData(1,"Store meshes in $PROJECT folder",Qt.ToolTipRole)
    ...
    def _write_to_slash_stage(self,mesh_data : str, mesh_type : str,mesh_name : str) -> None :
        ...
        Path(f"{dir}/{self.mesh_folder.text()}/{mesh_name}").mkdir(parents=True, exist_ok=True)

        out_name=f"{dir}/{self.mesh_folder.text()}/{mesh_name}/{mesh_name}.{mesh_type}"
    ...
    def _write_to_slash_obj(self,mesh_data : str, mesh_type : str,mesh_name : str) -> None :
        if self.mesh_location.currentIndex() == 0 :
            dir=hou.text.expandString("$HIP")
        else :
            dir=hou.text.expandString("$PROJECT")
