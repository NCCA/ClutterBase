    ...
    for file in files:
        if file.endswith(input):
            ...
            if output == "fbx":
                ...
                cmds.file(
                    f"{working_dir}/{out_file}",
                    options=";exportUVs=1;exportVisibility=1;mergeTransformAndShape=1;stripNamespaces=0;",
                    type="USD Export",
                    pr=True,
                    es=True,
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Batch Convert files valid formats are "obj","ma","mb","usd","usda","usdb","fbx"'
    )
